...
# Start YOLO Service (Python) on port 8000 (standard)
...
# Start Back-end (Node.js) on port 3001
...
# Start Front-end (React) on port 3000
Write-Host "[3/3] Starting Front-end React App on port 3000..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd 'C:\Users\ASUS\Desktop\Vision aid\front -end\vision-aid-ui'; `$env:PORT='3000'; `$env:BROWSER='none'; npm start" -WindowStyle Normal
...
Write-Host "  - Back-end API:    http://localhost:3001" -ForegroundColor White
Write-Host "  - Front-end App:   http://localhost:3000" -ForegroundColor White
